...
def test_ready_matches_artifacts():
    response = client.get("/ready", headers=TEST_HEADERS)
...
def test_model_info_contract_when_ready():
    ...
    response = client.get("/model-info", headers=TEST_HEADERS)
    assert response.status_code == 200
...
def test_predict_rejects_unknown_field():
    ...
    assert client.post("/predict", json=payload, headers=TEST_HEADERS).status_code == 422
