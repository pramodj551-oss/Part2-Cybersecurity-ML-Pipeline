...
class MetricsDict(dict):
    ...
    def __getitem__(self, key): return super().get(key, 0)
    def clear(self): super().clear(); super().update(self._defaults)

METRICS = MetricsDict(MetricsDict._defaults)
def _metric_value(name: str) -> int: return METRICS.setdefault(name, 0)
...
def _allow_request(client_id: str) -> tuple[bool, str]:
    now = int(time.time()); window = now // 60; key = f"{RATE_LIMIT_PREFIX}:{client_id}:{window}"
    try:
        client = _redis()
        if client is not None:
            count = int(client.incr(key))
            if count == 1: client.expire(key, 61)
            return count <= RATE_LIMIT, "redis"
    except Exception as exc:
        logger.warning(json.dumps({"event": "redis_rate_limit_unavailable", "error": type(exc).__name__}))
        if RATE_LIMIT_MODE == "redis": raise HTTPException(503, "Rate limiting backend unavailable") from exc
...
class PredictionRequest(BaseModel):
    ...
    records_affected: float = Field(ge=0); detection_time_hours: float = Field(ge=0); ransom_demand_usd: float = Field(ge=0)
    sector: str; region: str; attack_type: str; threat_actor: str; data_exfiltration: str | int | bool; zero_day_used: str | int | bool

class ModelRuntime:
    def __init__(self): self.model = None; self.preprocessor = None; self.metadata: dict[str, Any] = {}
...
# Compatibility-only snapshot for legacy imports/tests. Authentication never uses this stale value.
...
def require_api_key(request: Request, api_key: str | None) -> None:
    ...
    if not expected:
        ERROR_COUNT.labels(type="authentication_configuration").inc(); audit("authentication_configuration_missing", request); raise HTTPException(503, "Inference API authentication is not configured")
    if api_key != expected:
        ERROR_COUNT.labels(type="authentication").inc(); audit("authentication_failed", request); raise HTTPException(401, "Unauthorized")

def protected(request: Request, api_key: str | None):
    require_api_key(request, api_key)
    if request.url.path == "/predict":
        client_id = request.client.host if request.client else "unknown"; allowed, backend = _allow_request(client_id)
        if not allowed:
            ERROR_COUNT.labels(type="rate_limit").inc(); RATE_LIMIT_COUNT.labels(backend=backend).inc(); audit("rate_limit_exceeded", request, backend=backend); raise HTTPException(429, "Rate limit exceeded")

@app.middleware("http")
async def observe(request, call_next):
    start = time.perf_counter()
    try: response = await call_next(request)
    except Exception: ERROR_COUNT.labels(type="unhandled").inc(); raise
    elapsed = time.perf_counter() - start; REQUEST_COUNT.labels(request.method, request.url.path, str(response.status_code)).inc(); METRICS["requests_total"] = _metric_value("requests_total") + 1
    if response.status_code >= 400: METRICS["errors_total"] = _metric_value("errors_total") + 1
    REQUEST_LATENCY.labels(request.method, request.url.path).observe(elapsed); LATENCIES.append(elapsed); response.headers["X-Content-Type-Options"] = "nosniff"; response.headers["X-Frame-Options"] = "DENY"; response.headers["Cache-Control"] = "no-store"; audit("request", request, status_code=response.status_code, duration_ms=round(elapsed * 1000, 2)); return response

@app.get("/health")
def health(request: Request): return {"status": "ok", "timestamp": datetime.now(timezone.utc).isoformat()}

# Explicit alias removes any framework/header-name ambiguity in production and CI.
API_KEY_HEADER = "X-API-Key"

@app.get("/ready")
def ready(request: Request, x_api_key: str | None = Header(default=None, alias=API_KEY_HEADER)):
    protected(request, x_api_key)
    if not runtime.ready(): raise HTTPException(503, "Model artifacts are not ready")
    return {"status": "ready"}

@app.get("/metrics", include_in_schema=False)
def metrics(request: Request, x_api_key: str | None = Header(default=None, alias=API_KEY_HEADER)):
    protected(request, x_api_key)
    if request.query_params.get("format") == "json":
        for key in ("requests_total", "errors_total", "predictions_total"): _metric_value(key)
...
@app.get("/model-info")
def model_info(request: Request, x_api_key: str | None = Header(default=None, alias=API_KEY_HEADER)):
    protected(request, x_api_key)
    if not runtime.ready(): raise HTTPException(503, "Model artifacts are not ready")
    metadata = json.loads(Path(MODEL_METADATA_FILE).read_text(encoding="utf-8")); return {"model": metadata.get("model_name"), "metrics": {key: metadata.get(key) for key in ("r2_score", "rmse", "mae", "cv_mean")}, "required_features": REQUIRED_FEATURES}

@app.post("/predict")
def predict(request: Request, payload: PredictionRequest, x_api_key: str | None = Header(default=None, alias=API_KEY_HEADER)):
    protected(request, x_api_key)
    try:
        score = runtime.predict(payload.model_dump()); PREDICTION_COUNT.labels(outcome="success").inc(); METRICS["predictions_total"] = _metric_value("predictions_total") + 1; audit("prediction", request, model=runtime.metadata.get("model_name")); return {"prediction": score, "model": runtime.metadata.get("model_name")}
    except FileNotFoundError as exc: ERROR_COUNT.labels(type="artifacts").inc(); PREDICTION_COUNT.labels(outcome="error").inc(); raise HTTPException(503, str(exc)) from exc
    except ValueError as exc: ERROR_COUNT.labels(type="validation").inc(); PREDICTION_COUNT.labels(outcome="error").inc(); raise HTTPException(422, str(exc)) from exc
    except HTTPException: raise
    except Exception as exc: ERROR_COUNT.labels(type="prediction").inc(); PREDICTION_COUNT.labels(outcome="error").inc(); logger.exception("prediction_failed"); raise HTTPException(500, "Prediction failed") from exc
