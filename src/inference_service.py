...
app = FastAPI(title="Cybersecurity ML Inference Service", version="23.1")
...
class ModelRuntime:
    ...
    def ready(self) -> bool:
        return (
            ...
            and MODEL_METADATA_FILE.exists()
        )
    ...
    def load(self):
        ...
        self.metadata = json.loads(
            Path(MODEL_METADATA_FILE).read_text(encoding="utf-8")
        )
        return self
    ...
    def predict(self, payload: dict) -> float:
        ...
        if selected:
            ...
            if missing:
                raise ValueError(
                    f"Model contract mismatch: missing selected features {missing}"
                )
...
def require_api_key(provided_api_key: str | None) -> None:
    """Require a configured server API key and reject missing/incorrect credentials."""
    expected_api_key = os.getenv("INFERENCE_API_KEY")
    if not expected_api_key:
        raise HTTPException(
            status_code=503,
            detail="Inference API authentication is not configured",
        )
...
@app.get("/health")
def health():
    """Public liveness endpoint; readiness and inference remain protected."""
    return {"status": "ok", "timestamp": datetime.now(timezone.utc).isoformat()}


@app.get("/ready")
def ready(x_api_key: str | None = Header(default=None)):
    require_api_key(x_api_key)
    if not runtime.ready():
        raise HTTPException(
            status_code=503, detail="Model artifacts are not ready"
        )
    return {"status": "ready"}
...
def model_info(x_api_key: str | None = Header(default=None)):
    ...
    return {
        ...
        "metrics": {
            key: metadata.get(key) for key in ("r2_score", "rmse", "mae", "cv_mean")
        },
    ...
    try:
        ...
        logger.info(
            json.dumps(
                {
                    "event": "prediction",
                    "model": runtime.metadata.get("model_name"),
